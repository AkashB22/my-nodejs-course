...
//Error Page
handler.errorPage = function(data, callback){
    var error = new Error("This is a sample Error");
    throw error;
};
...
handler._token.post = function(data, callback){
    _performance.mark('starting of token post');
    var phone = typeof(data.payload.phone) == 'string' && data.payload.phone.trim().length == 10 ? data.payload.phone.trim() : '';
    var password = typeof(data.payload.password) == 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : '';
    _performance.mark('validated user inputs');
    if(phone && password){
        _performance.mark('starting to look into the users');
        //lookup the user who matches that phone
        _data.read('user', phone, function(err, userData){
            _performance.mark('end of the user lookup');
            if(!err && userData){
                _performance.mark('beginning of the hashing password');
                var hashedPassword = helpers.hash(password);
                _performance.mark('end of the password hashing');
                if(hashedPassword == userData.hashedPassword){
                    ...
                    _performance.mark('created token data');
                    _performance.mark('starting to store token data');
                    _data.create('token', tokenId, tokenObject, function(err){
                        _performance.mark('stored token data');

                        //measuring performance
                        _performance.measure('complete token post', 'starting of token post', 'stored token data');
                        _performance.measure('user input  validation', 'starting of token post', 'validated user inputs');
                        _performance.measure('user lookup completed', 'starting to look into the users', 'end of the user lookup');
                        _performance.measure('hashing password', 'beginning of the hashing password', 'end of the password hashing');
                        _performance.measure('token data creation', 'creating token data', 'created token data');
                        _performance.measure('Storing of token data', 'starting to store token data', 'stored token data');

                        //displaying measure duration and name
                        measurements = _performance.getEntriesByType('measure');
                ...
                _data.read('user', userPhone, function(err, userData){
                    if(!err && userData){
                        ...
                        if(userChecks.length < config.maxChecks){
                            ...
                            var parsedUrl = _url.parse(protocol + '://' + url);
                            ...
                            _data.create('check', checkId , checkObject, function(err){
                                if(!err){
                                    ...
                                    callback(true, {'Error' : 'The HostName given is not resolved to any DNS entries'})
                                }
                            });
                        } else {
